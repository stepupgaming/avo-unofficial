...
function pickOp(memory) {
  ...
  if (kHints.length) return { op: kHints[0], reason: 'k_prior' }
  for (const key of weak) {
    for (const op of map[key] || []) {
      if (!tried.has(op) && !committed.has(op) && op !== 'h3_regen') return { op, reason: 'weak:' + key }
    }
  }
  for (const op of cheap) {
    if (!tried.has(op) && !committed.has(op) && op !== 'h3_regen') return { op, reason: 'untried_cheap' }
  }
...
export class Agent {
  ...
  decide() {
    ...
    if (!this.memory.diagnosed) return { tool: 'diagnose' }
  ...
  run(inner = 8) {
    ...
    for (let i = 0; i < inner; i++) {
      ...
      if (step.tool === 'diagnose') { this.diagnose(this.memory.parentScore); continue }
      if (step.tool === 'mutate') {

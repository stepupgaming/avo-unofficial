...
export function renderProxy(edl, outPath) {
  ...
  if (!aroll) throw new Error('no resolvable a-roll')
...
export function measureEdl(edl) {
  ...
  try {
    if (!existsSync(out)) renderProxy(edl, out)
...
export function blendMeasured(dummy, measured, edl) {
  ...
  for (const c of edl?.tracks?.video || []) {
    if (Number(c.crop || 1) > 1.05) firstVisual = Math.min(firstVisual, Number(c.t0 || 0))
  }
  for (const c of edl?.tracks?.broll || []) firstVisual = Math.min(firstVisual, Number(c.t0 || 0))
  ...
  if (firstVisual > 2.2) visual = Math.max(0, visual - 0.2)
  let pacing = dummy.vector.pacing
  if (cps > 1.2) pacing = Math.min(pacing, 0.45)
  if (cps === 0 && d > 20) pacing = Math.min(pacing, 0.35)
  const vector = {
    ...dummy.vector,

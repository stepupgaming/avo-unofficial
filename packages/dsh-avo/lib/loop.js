...
export function correctness(edl) {
    ...
    if (vs.length && as_.length) {
        if (Math.abs(Math.min(...vs) - Math.min(...as_)) > 0.25)
            reasons.push('desync');
        if (Math.abs(Math.max(...ve) - Math.max(...ae)) > 0.25)
            reasons.push('desync');
    }
...
export function loadKnowledge(genomeDir = '/workspace/editing-genome') {
    ...
    if (existsSync(priors)) {
        ...
        if (hook?.must_change_visual_or_type_by_s != null)
            k.hook_visual_by_s = Number(hook.must_change_visual_or_type_by_s);
...
export function evaluate(edl, k) {
    ...
    for (const c of videos) {
        if (Number(c.crop || 1) > 1.05 || Number(c.t0 || 0) > 0)
            firstChange = Math.min(firstChange, Number(c.t0 || 0));
    }
    for (const c of [...broll, ...graphics, ...captions])
        firstChange = Math.min(firstChange, Number(c.t0 || 0));
    ...
    if (firstChange > 2.2)
        visualNovelty = clip(visualNovelty - 0.25);
    const d = durationS(edl);
    let pacing = d >= 25 && d <= 45 ? 0.9 : d >= 15 && d <= 60 ? 0.55 : 0.2;
    const counts = eventCount(edl);
    if (counts.video_clips + counts.broll + counts.punch_ins === 1 && d > 20)
        pacing = Math.min(pacing, 0.4);
    if (counts.punch_ins > 2 || counts.sfx > 3 || counts.captions > 4)
        pacing = Math.min(pacing, 0.5);
    let av = sfx.length ? 0.75 : 0.4;
    if ((tracks.audio || []).some((x) => x.duck))
        av = Math.min(1, av + 0.15);
    let yt = 0.4;
    if (d >= 20 && d <= 45)
        yt += 0.25;
    if (captions[0] && Number(captions[0].t0 ?? 99) <= 1)
        yt += 0.25;
    yt = clip(yt);
    const positives = {
    ...
    let over = 0;
    if (counts.captions > (bands.captions ?? 6))
        over += 0.25;
    if (counts.sfx > (bands.sfx ?? 4))
        over += 0.25;
    if (counts.punch_ins > (bands.punch_ins ?? 3))
        over += 0.25;
    ...
    if (counts.graphics > 4)
        distraction = clip(distraction + 0.2);
...
function tracks(edl) {
    ...
    for (const k of ['video', 'audio', 'captions', 'graphics', 'sfx', 'broll'])
        edl.tracks[k] = edl.tracks[k] || [];
    return edl.tracks;
}
export function applyMutation(edl, op) {
    if (op === 'h3_regen')
        throw new Error(H3_REFUSED);
    const out = clone(edl);
    const t = tracks(out);
    if (op === 'trim') {
        const t1 = 32;
    ...
    if (op === 'music_duck') {
        if (t.audio[0])
            ...
            t.audio.push({ id: 'a-duck', src: 'fixture://vo', t0: 0, t1: 32, duck: true });
        return out;
    }
...
export class Lineage {
    ...
    best() {
        if (!this.index.committed.length)
            return null;
...
function diagnostics(score) {
    ...
    if ((v.attention_support ?? 1) < 0.7)
        d.push('hook_needs_visual');
    if ((v.semantic_alignment ?? 1) < 0.5)
        d.push('claim_unpaired');
    if ((v.pacing ?? 1) < 0.6)
        d.push('flat_pacing');
    if ((v.youtube_prior ?? 1) < 0.6)
        d.push('weak_packaging');
    if ((score.counts?.captions ?? 0) === 0)
        d.push('no_captions');
    return d;
}
...
export function varyOnce(seed, lineage, k, inner = 5) {
    ...
    if (!lineage.index.committed.length)
        lineage.commit(parent, parentScore, 'seed');
    const tried = [];
    let force = null;
...
export function runVary({ fixture, lineageDir, steps = 1, genomeDir }) {
    ...
    for (let i = 0; i < steps; i++)
        results.push(varyOnce(seed, lineage, k, 5));
    return results;
}

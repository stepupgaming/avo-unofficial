...
export function classifyCut(path, t, fps, tmpDir) {
    ...
    if (!extractFrame(path, t - dt, pre) || !extractFrame(path, t + dt, post))
        return { kind: 'hard_cut', params: { detect: 'scene_select', note: 'frame_extract_failed' } };
    spawnSync('ffmpeg', [
        '-hide_banner', '-y', '-i', pre,
    ...
    if (full < 8)
        return { kind: 'match_or_soft_cut', params: { detect: 'low_luma_delta' } };
    return { kind: 'hard_cut', params: { detect: 'scene_select' } };
}
...
function beatGuess(beats, t) {
    for (const b of beats) {
        if (b.t_start <= t && t < b.t_end)
            return b.beat;
        if (b.beat === 'close' && t >= b.t_start)
            return b.beat;
    }
...
function speechAt(segments, t0, t1) {
    if (!segments?.length)
        return { speech: null, speech_pace: null };
...
function recountSummary(outDir) {
    ...
    if (existsSync(vFile))
        videos = readFileSync(vFile, 'utf8').split('\n').filter((l) => l.trim()).length;
    if (existsSync(bFile))
        beats = readFileSync(bFile, 'utf8').split('\n').filter((l) => l.trim()).length;
...
function extractWav(path, wavPath) {
    ...
    if (r.status !== 0 || !existsSync(wavPath))
        throw new Error((r.stderr || 'wav extract failed').slice(-400));
    ...
    if (r2.status !== 0 || !existsSync(clipped))
        throw new Error('wav clip failed');
    return clipped;
}
async function groqAsr(wavPath) {
    ...
    if (!res.ok)
        throw new Error(`groq asr ${res.status}: ${(await res.text()).slice(0, 400)}`);
    return res.json();
}
async function loadOrRunAsr(path, videoId, outDir, noAsr) {
    if (noAsr || !process.env.GROQ_API_KEY)
        return null;
    ...
    try {
        ...
        try {
            if (used !== wavPath && existsSync(wavPath))
                unlinkSync(wavPath);
        }
        catch { /* ignore */ }
    }
...
export async function ingestFile(file, opts = {}) {
    ...
    try {
        ...
        for (const t of boundaries) {
            if (!bnds.length || t - bnds[bnds.length - 1] > 0.04)
                bnds.push(t);
        }
        if (bnds[bnds.length - 1] < meta.duration - 0.01)
            bnds.push(meta.duration);
        let prevAction = null;
    ...
    for (const e of events)
        e.content_state.beat_guess = beatGuess(beatRows, e.t);
    const videoRow = {
        id,
        url: null,
...
export async function ingest(input, opts = {}) {
    ...
    for (const f of files)
        results.push(await ingestFile(f, opts));
    return results;
}
export { PUNCHIN_CROP };
